...
func query(db *gorm.DB) error {
	...
	fmt.Println("A KioskEvent's Checkins")
...
func create(db *gorm.DB) error {
	...
	err = createCheckin(db, 2, 1, "third checkin")
	if err != nil {
		return err
	}
	err = createCheckin(db, 1, 2, "fourth checkin")
	if err != nil {
		return err
	}
	err = createCheckin(db, 2, 1, "fifth checkin")
	if err != nil {
		return err
	}
...
func addBadCheckin(db *gorm.DB) error {
	...
	fmt.Println("error expected for an insert on table \"checkins\"")
